...
![MyRelational Database Management SystemSQL](https://upiclw.oss-cn-beijing.aliyuncs.com/uPic/QTcJZh.png)
...
WHERE id = (SELECT id from user2 WHERE user2.name='小帆');
...
```sql
-- 向表 Persons 插入一条字段 LastName = FAN 字段 Address = shanghai
INSERT INTO Persons (LastName, Address) VALUES ('FAN', 'shanghai');
...
INSERT INTO `charger` (`id`,`type`,`create_at`,`update_at`) VALUES (3,2,'2025-03-18 11:06:17','2025-03-18 11:06:17') 
...
-- 删除 Person 表字段 LastName = 'Fan' 
DELETE FROM Person WHERE LastName = 'Fan' 
...
```sql 
-- 从表 Persons 中选出 Year 字段大于 2000 的数据
SELECT * FROM Persons WHERE Year>2000;

-- 从 Customers 表中选择 Country = China 的所有数据：
...
SQL 使用单引号来环绕文本值（大部分数据库系统也接受双引号）。如果是`数值`，请不要使用引号。
...
-- 从 Customers 表中选择 Country 不是 Germany 和 不是 "USA" 的所有字段：
...
`IN 语法` 运算符允许在 WHERE 子句中指定多个值。运算符是多个 `OR` 条件的简写。
...
FROM meeting 
UNION ALL 
SELECT id,number_station AS pic_url 
FROM station  
ORDER BY update_at;

-- 通过 UNION 语法同时查询了 products 表 和 comments 表的总记录数，并且按照 count 排序
SELECT 'product' AS type, count(*) as count 
FROM `products` 
union 
select 'comment' as type, count(*) as count 
FROM `comments` 
order by count;
```
...
- https://blog.csdn.net/Innocence_0/article/details/132438303?ops_request_misc=%257B%2522request%255Fid%2522%253A%252269534d33c612e7e892fa3179c8286eea%2522%252C%2522scm%2522%253A%252220140713.130102334..%2522%257D&request_id=69534d33c612e7e892fa3179c8286eea&biz_id=0&utm_medium=distribute.pc_search_result.none-task-blog-2~all~top_positive~default-2-132438303-null-null.142^v101^pc_search_result_base3&utm_term=sql&spm=1018.2226.3001.4187
